...
function renderOpsCenter(){
  ...
  document.getElementById('agents').innerHTML='<div class="empty">运行时状态、任务注册表状态和最后事件已合并到 Agent 舰队。</div>';
...
function renderOverview(){
  ...
  document.getElementById('projects').innerHTML=ss.length?ss.map(s=>`
    <button class="project ${s.workspace_id===state.spaceId?'active':''}" onclick="selectSpace('${esc(s.workspace_id)}')">
      ...
      <small>${esc(s.workspace_id)} · ${s.tab_count||0} 个 Tab · ${s.pane_count||0} 个 Pane</small>
  ...
  if(s.relation==='current_factory'&&s.project_id){
    ...
    document.getElementById('tasks').innerHTML=`
      <div class="empty">
        ...
        <div style="margin-top:4px">${s.tab_count||0} 个 Tab · ${s.pane_count||0} 个 Pane</div>
