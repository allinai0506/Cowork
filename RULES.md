...
开发任务必须严格按照以下四个阶段顺序闭环执行，严禁跳步或无方案直接开工：

```mermaid
graph LR
    P[1. Plan 规划] --> T[2. TDD 开发]
...
### 🔴 空间隔离红线
- **严禁在主干工作区直接修改代码执行业务 Task**：
  - 任何针对研发项目的 Task 派发必须使用 `herdr-task launch`。
  - 所有 Agent 必须在独立的 CoW Clone（`~/.herdr-controller/clones/<task-id>`）中隔离运行。
