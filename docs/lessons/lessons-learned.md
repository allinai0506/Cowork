...
在项目的迭代演进中，主文档（如 `README.md`）及多处核心指引逐渐滋生出带有“过度包装”、“绝对化承诺”与“不可控 SLA”的工程表述：
...
1. **绝对化用语一律清除**：主干 README 与官方文档严禁出现“100% 兼容”、“绝对无死角”、“永不中断”等不可控断言；
2. **拒绝制造外部 SLA 承诺**：内部调度机制描述应强调“触发时机”与“动作确定性”（例如“任务派发前自动检测并修复”），严禁使用“毫秒级”、“瞬时”等易受环境干扰的时间承诺修饰；
3. **精准声明兼容范围**：兼容性说明必须具体指向具体的存量实体（如 legacy stage-based workflow、旧版 `--stage` 别名），而非含混的绝对兼容；
4. **统一文档协同一致性**：当主 README 调整核心特性的宣称口径时，必须同步核验并对齐架构文档、开发者手册与 Wiki 索引。

### 验证命令 / 证据

```bash
# 1. 检查主文档与全仓核心文档，确认危险承诺关键词已收敛
git grep -iE "(100% 向下兼容|100% 兼容|毫秒级自动修复|毫秒级自动自愈|操作系统底座)" README.md docs/ wiki/
